        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <ul className="list-disc pl-4 text-xs text-muted-foreground space-y-1.5">
              ...
              <li><strong>Tabs 模块化拆分</strong>：凡是带有多个 Tab 页切换的页面，**禁止**将所有 Tab 的渲染代码堆积在同一个物理页面主文件内。每个 Tab 页对应的 Content 必须单独封装进全局 <code className="bg-muted px-1 py-0.5 rounded font-mono">components/</code> 目录下专属文件夹中，主页面文件只负责维护当前 Tab 的激活状态及外层骨架布局。这有利于模块复用、编译优化以及提升单文件可读性。</li>
              <li><strong>复杂度驱动的区块拆分</strong>：不仅是 Tabs 切换，当一个页面为单页界面且代码行数超过 800 行时，必须将不同的物理块内容拆分为子组件。为保持路由树的清爽和逻辑纯粹，所有拆分出的非复用子组件也应统一存放在全局组件文件夹中（如 <code className="bg-muted px-1 py-0.5 rounded font-mono">components/common/admin/</code>），而路由入口 <code className="bg-muted px-1 py-0.5 rounded font-mono">page.tsx</code> 仅用于定义整个页面的布局排版与全局框架。典型标杆案例参考“数据管理” (<code className="bg-muted px-1 py-0.5 rounded font-mono">/admin/database</code>) 的重构。</li>
            </ul>
          </div>
        </CardContent>
      </Card>
